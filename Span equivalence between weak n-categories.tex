...
Thomas Cottrell  showed the following cohorence theorem for some weak $n$-categories.
\begin{theorem}
...
\end{theorem}
The equivalence in this theorem is defined as follows:
\begin{definition}
...
\end{definition}
This equivalence is defined for the coherence theorem above and unsatisfactory equivalence for $K$-algebras (or weak $n$-categories) as he say: ``this definition of equivalence is much more strict (and thus much less general) than ought to be.'' 
He proposed two approaches for a more general definition of equivalence of $K$-algebras. One of them is to replace the map $u$ with a span of maps of $K$-algebras. In this paper, we will see two result for one. First result, span euivalent is equivalence relation. Second result, span equivalence is generalized ordinary equivalence. \\
In Section 2 we recall the definition of globular sets, thier maps, operads, algebras for a operad. In Section 3 we define span equivalence and prove first result. In Section 4 we define span equivalence of ordinary categories and equivalence fusion, then we prove second result.
...
\end{definition}
%K-algの射についても同様に定義する。の文面を加える。
%K is a monad on n-GSet. 
%A map of K-algebras is surjective on k-cells (resp. injective on k-cells, etc) if the underlying map is surjective on k-cells (resp injective on k-cells, etc)
